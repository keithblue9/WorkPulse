...
function AddIssueModal({ onClose, onSave }: { onClose:()=>void; onSave:()=>void }) {
  ...
  const [form, setForm] = useState({ initiativeId:'', title:'', description:'', progress:0, status:'on_track', nextPlan:'', dueDate:'', pic:'', picName:'' })
  ...
  async function save() {
    ...
    try {
      ...
      toast.success('Issue berhasil ditambahkan!'); onSave(); onClose()
    ...
    <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{ width:540 }}>
        ...
        <div style={{ padding:'16px 20px', display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div><label style={lbl}>Judul Issue *</label><input className="input" value={form.title} onChange={e=>set('title',e.target.value)} placeholder="Deskripsi singkat issue..." /></div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div><label style={lbl}>PIC (Nama)</label><input className="input" value={form.picName} onChange={e=>{set('picName',e.target.value);set('pic',e.target.value)}} placeholder="Nama PIC..." /></div>
            <div><label style={lbl}>Progress Awal (%)</label><input type="number" min={0} max={100} className="input" value={form.progress} onChange={e=>set('progress',Number(e.target.value))} /></div>
          </div>
    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:'var(--bg)' }}>
      ...
      <aside style={{ width:collapsed?56:220, background:'var(--bg2)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', flexShrink:0, transition:'width 0.2s ease', overflow:'hidden' }}>
        ...
        <nav style={{ padding:'8px', flex:1, overflowY:'auto' }}>
          {NAV_GROUPS.map(group => {
            ...
            return (
              <div key={group.label}>
                {!collapsed && <div style={{ fontSize:9, fontWeight:700, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.08em', padding:'10px 8px 4px' }}>{group.label}</div>}
                {visibleItems.map(item => {
                  const active = pathname===item.href||(item.href!=='/dashboard'&&pathname.startsWith(item.href))
                  return (
                    <Link key={item.href} href={item.href} style={{ textDecoration:'none' }}>
                      <div className={`sidebar-link${active?' active':''}`} style={{ justifyContent:collapsed?'center':'flex-start', padding:collapsed?'10px':'7px 10px' }} title={item.label}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <header style={{ display:'flex', alignItems:'center', gap:8, padding:'0 16px', height:52, borderBottom:'1px solid var(--border)', background:'var(--bg2)', flexShrink:0 }}>
          ...
          <div ref={themeRef} style={{ position:'relative' }}>
            ...
            {showThemePicker && (
              <div className="scale-in" style={{ position:'absolute', top:'calc(100% + 6px)', right:0, background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:10, padding:6, zIndex:200, minWidth:160, boxShadow:'var(--shadow)' }}>
                <div style={{ padding:'4px 10px 6px', fontSize:10, fontWeight:600, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.06em' }}>Theme</div>
                {THEMES.map(t => (
          ...
          <button className="btn btn-sm btn-primary" onClick={()=>setShowAddIssue(true)}>+ Tambah Issue</button>
        </header>
