...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden', background:'var(--bg)', padding:24 }} className="safe-all">
      ...
      {!hasBg && (
        <div className="ambient-bg">
          ...
          <div className="orb" style={{ width:400, height:400, bottom:'-15%', right:'15%', animationDelay:'-8s' }} />
        </div>
      )}

      {/* Apple-style transparent card */}
      <div style={{
        ...
        <div style={{ marginBottom:24, textAlign:'center' }}>
          {config.appIcon ? (
            <img src={config.appIcon} alt="logo" style={{ width:60, height:60, borderRadius:16, objectFit:'cover', marginBottom:14 }} />
          ) : (
            <div style={{ width:60, height:60, borderRadius:16, background:appColor, display:'inline-flex', alignItems:'center', justifyContent:'center', fontSize:30, fontWeight:700, color:'#fff', marginBottom:14, boxShadow:`0 10px 30px ${appColor}50` }}>{(config.appName||'W')[0]}</div>
          )}
          <div style={{ fontSize:26, fontWeight:700, letterSpacing:'-0.02em', marginBottom:4, color:'#fff' }}>{config.appName||'WorkPulse'}</div>
          <div style={{ fontSize:12, color:'rgba(255,255,255,0.6)' }}>{config.loginTagline}</div>
        </div>

        <div ref={pickerRef} style={{ position:'relative', marginBottom:18 }}>
          <label style={{ display:'block', fontSize:11, fontWeight:500, color:'rgba(255,255,255,0.7)', marginBottom:6 }}>Akun</label>
          <button onClick={()=>setShowUserPicker(s=>!s)} type="button" style={{
            width:'100%', padding:'10px 12px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10,
            display:'flex', alignItems:'center', gap:10, cursor:'pointer', color:'#fff',
          }}>
            {selectedUser ? (
              <>
                ...
                <div style={{ flex:1, textAlign:'left', minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:600, color:'#fff', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{selectedUser.name}</div>
                  <div style={{ fontSize:10, color:'rgba(255,255,255,0.55)' }}>{selectedUser.division || selectedUser.role}</div>
                </div>
              </>
            ) : (
              <span style={{ flex:1, textAlign:'left', color:'rgba(255,255,255,0.5)', fontSize:13 }}>Pilih nama akun...</span>
            )}
            <span style={{ color:'rgba(255,255,255,0.5)', fontSize:10 }}>▼</span>
          </button>
          {showUserPicker && (
            <div className="scale-in" style={{ position:'absolute', top:'calc(100% + 6px)', left:0, right:0, maxHeight:260, overflowY:'auto', borderRadius:12, padding:6, zIndex:10, background:'rgba(28,30,40,0.95)', backdropFilter:'blur(40px) saturate(180%)', border:'1px solid rgba(255,255,255,0.1)' }}>
              {users.map(u => (
                <div key={u.email} onClick={()=>selectUser(u)} style={{ padding:'8px 10px', borderRadius:8, cursor:'pointer', display:'flex', alignItems:'center', gap:10, background: selectedUser?.email===u.email?'rgba(255,255,255,0.08)':'transparent', color:'#fff' }}
                  onMouseEnter={e=>{if(selectedUser?.email!==u.email)(e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.05)'}}
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:10, color:'rgba(255,255,255,0.5)' }}>{u.division || u.role}</div>
                  </div>
                  {selectedUser?.email===u.email && <span style={{ color:appColor }}>✓</span>}
                </div>
              ))}
              {users.length === 0 && <div style={{ padding:14, textAlign:'center', color:'rgba(255,255,255,0.5)', fontSize:11 }}>Belum ada user</div>}
            </div>
          )}
        </div>

        <div style={{ marginBottom:18 }}>
          <label style={{ display:'block', fontSize:11, fontWeight:500, color:'rgba(255,255,255,0.7)', marginBottom:6 }}>PIN 6 Digit</label>
          <div style={{ display:'flex', gap:8, justifyContent:'space-between' }}>
            {pin.map((d,i) => (
              <input key={i} ref={el=>{inputRefs.current[i]=el}} type="tel" inputMode="numeric" pattern="[0-9]" maxLength={1} value={d}
                ...
                style={{ width:'100%', height:48, textAlign:'center', fontSize:20, fontWeight:600, border:'1px solid rgba(255,255,255,0.12)', borderRadius:10, background:'rgba(255,255,255,0.05)', color:'#fff', outline:'none' }}
        ...
        <button onClick={()=>submitWithPin(pin.join(''))} disabled={loading || !selectedUser || pin.some(d=>!d)}
          style={{ width:'100%', padding:'12px', fontSize:13, fontWeight:600, background:appColor, color:'#fff', border:'none', borderRadius:10, cursor:'pointer', opacity:(loading||!selectedUser||pin.some(d=>!d))?0.5:1 }}>
          {loading?'Masuk...':'Masuk'}
        </button>
        <div style={{ marginTop:14, paddingTop:12, borderTop:'1px solid rgba(255,255,255,0.08)', textAlign:'center' }}>
          <div style={{ fontSize:10, color:'rgba(255,255,255,0.5)' }}>PIN default: <code style={{ color:'rgba(255,255,255,0.75)' }}>123456</code> · Ubah di Profil</div>
        </div>
      </div>
    </div>
  )
