...
// A user is "guest/external-only" if every role they hold is 'guest'. These accounts
// are NOT shown in the Internal picker — external access is via the front-door button.
function isGuestOnly(u: any): boolean {
  const r = (u?.roles && u.roles.length) ? u.roles : (u?.role ? [u.role] : [])
  return r.length > 0 && r.every((x: string) => x === 'guest')
}

export default function LoginPage() {
  const [mode, setMode] = useState<'choose' | 'internal'>('choose')
  ...
  useEffect(() => {
    ...
    fetch('/api/users').then(r => r.json()).then(d => {
      const active = (d.data || []).filter((u: any) => u.active !== false && !isGuestOnly(u))
      setUsers(active)
  ...
  async function handleGuest() {
    ...
    const res = await signIn('credentials', { guest: 'true', redirect: false })
    setGuestLoading(false)
    if (res?.ok) { router.push('/dashboard') }
    else { toast.error('Gagal masuk sebagai External') }
  }
  ...
  function backToChoose() {
    setMode('choose'); setShowUserPicker(false); setPin(['', '', '', '', '', ''])
  }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden', background: '#f5f5f7', padding: '24px 5vw' }} className="safe-all login-shell">
      ...
      <div className="login-card" style={{
        ...
        {mode === 'choose' ? (
          /* ── Step 1: pilih jenis akses ── */
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'rgba(0,0,0,0.6)', textAlign: 'center', marginBottom: 2 }}>Masuk sebagai</div>
            <button onClick={() => setMode('internal')}
              style={{ width: '100%', padding: '14px', fontSize: 14, fontWeight: 700, background: appColor, color: '#fff', border: 'none', borderRadius: 12, cursor: 'pointer', boxShadow: `0 6px 16px ${appColor}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              👤 Internal
            </button>
            <button onClick={handleGuest} disabled={guestLoading}
              style={{ width: '100%', padding: '14px', fontSize: 14, fontWeight: 700, background: 'rgba(255,255,255,0.7)', color: '#1a1a1a', border: '1px solid rgba(0,0,0,0.12)', borderRadius: 12, cursor: 'pointer', opacity: guestLoading ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              {guestLoading ? 'Masuk...' : '🌐 External (Guest)'}
            </button>
            <div style={{ fontSize: 10, color: 'rgba(0,0,0,0.45)', textAlign: 'center', marginTop: 4 }}>External masuk langsung tanpa PIN (akses terbatas)</div>
          </div>
        ) : (
          /* ── Step 2: login Internal (picker + PIN) ── */
          <>
            <button onClick={backToChoose} type="button"
              style={{ background: 'transparent', border: 'none', color: 'rgba(0,0,0,0.55)', fontSize: 12, fontWeight: 600, cursor: 'pointer', padding: 0, marginBottom: 12 }}>← Kembali</button>
