...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async session({ session, token }: any) {
      if (session.user) { (session.user as any).role = token.role; (session.user as any).roles = token.roles; (session.user as any).division = token.division; (session.user as any).id = token.id }
      return session
    },
  },
  pages: { signIn: '/login' },
